export const realCopy: Array<[string, string, string]> = [
  ["Market", "Mercado", "Marché"],
  ["Forge", "Forja", "Forge"],
  ["Pricing", "Precios", "Tarifs"],
  ["Bounded source synthesis", "Síntesis acotada de fuentes", "Synthèse bornée des sources"],
  ["Observed opportunities and auditable receipts. Underwriting stays execution_not_enabled; one separate user-authorized public-source synthesis route is available. No marketplace actions or payments.", "Oportunidades observadas y recibos auditables. La evaluación mantiene execution_not_enabled; hay una ruta separada de síntesis de fuentes públicas autorizada por el usuario. Sin acciones de mercado ni pagos.", "Opportunités observées et reçus auditables. L’analyse conserve execution_not_enabled ; une route distincte de synthèse de sources publiques autorisée par l’utilisateur est disponible. Aucune action de marché ni paiement."],
  ["Public-source synthesis is opt-in: POST /api/v1/forge/synthesize with 1–10 public HTTPS URLs, a task objective, a unique run ID, and explicit run_task authorization. One bounded Groq call returns cited findings and a separate execution receipt. No marketplace work is executed.", "La síntesis de fuentes es opcional: POST /api/v1/forge/synthesize con 1–10 URL HTTPS públicas, un objetivo, un ID único y autorización explícita run_task. Una llamada acotada a Groq devuelve hallazgos con fuentes y un recibo de ejecución separado. No se ejecuta trabajo de mercado.", "La synthèse des sources est facultative : POST /api/v1/forge/synthesize avec 1 à 10 URL HTTPS publiques, un objectif, un ID unique et l’autorisation explicite run_task. Un appel Groq borné renvoie des constats sourcés et un reçu d’exécution distinct. Aucune mission de marché n’est exécutée."],
  ["Claim readiness inspection", "Inspección de preparación para reclamar", "Inspection de préparation à la réclamation"],
  ["Authorization", "Autorización", "Autorisation"],
  ["REQUIRED / NOT GRANTED", "REQUERIDA / NO CONCEDIDA", "REQUISE / NON ACCORDÉE"],
  ["Missing inputs", "Datos faltantes", "Données manquantes"],
  ["A public Coinbase spot-price endpoint may be queried for USDC/USD. SignalForge caches the rate, timestamp and source metadata, never account data. Operator economic assumptions are processed for the requested receipt and are not intentionally persisted. Manual outcome records remain schema-only in this milestone. Shared Redis keys are isolated by a trusted production, preview, development or test namespace.", "SignalForge puede consultar un endpoint público de precio spot de Coinbase para USDC/USD. Almacena en caché el tipo, la fecha y los metadatos de la fuente, nunca datos de cuenta. Los supuestos económicos del operador se procesan para el recibo solicitado y no se conservan intencionalmente. Los resultados manuales siguen siendo solo un esquema en este hito. Las claves Redis compartidas se aíslan mediante un espacio de nombres confiable de producción, vista previa, desarrollo o pruebas.", "SignalForge peut interroger un endpoint public de prix spot Coinbase pour USDC/USD. Le taux, l’horodatage et les métadonnées de source sont mis en cache, jamais les données de compte. Les hypothèses économiques de l’opérateur sont traitées pour le reçu demandé et ne sont pas conservées intentionnellement. Les résultats manuels restent limités au schéma dans ce jalon. Les clés Redis partagées sont isolées par un espace de noms fiable de production, prévisualisation, développement ou test."],
  ["POST /api/v1/opportunities/claim-readiness — read-only economic and authorization inspection; claimAuthorized is always false.", "POST /api/v1/opportunities/claim-readiness — inspección económica y de autorización de solo lectura; claimAuthorized siempre es false.", "POST /api/v1/opportunities/claim-readiness — inspection économique et d’autorisation en lecture seule ; claimAuthorized est toujours false."],
  ["Complete the economics", "Completa la economía", "Complétez les paramètres économiques"],
  ["Apply assumptions", "Aplicar supuestos", "Appliquer les hypothèses"],
  ["Check the bounded numeric assumptions.", "Revisa los supuestos numéricos acotados.", "Vérifiez les hypothèses numériques bornées."],
  ["Max input tokens", "Máximo de tokens de entrada", "Jetons d’entrée maximum"],
  ["Max output tokens", "Máximo de tokens de salida", "Jetons de sortie maximum"],
  ["Bounded model calls", "Llamadas de modelo acotadas", "Appels de modèle bornés"],
  ["Platform fee", "Comisión de plataforma", "Frais de plateforme"],
  ["Proof / gas fee", "Coste de prueba / gas", "Frais de preuve / gas"],
  ["Human review cost", "Coste de revisión humana", "Coût de révision humaine"],
  ["Additional fulfillment", "Ejecución adicional", "Réalisation supplémentaire"],
  ["Time-value cost", "Coste temporal", "Coût de la valeur temps"],
  ["Competition-risk adjustment", "Ajuste por riesgo competitivo", "Ajustement du risque concurrentiel"],
  ["Bond loss probability", "Probabilidad de pérdida de fianza", "Probabilité de perte de la caution"],
  ["USDC / USD scenario rate", "Tipo de escenario USDC / USD", "Taux de scénario USDC / USD"],
  ["Expected profit", "Beneficio esperado", "Profit attendu"],
  ["Expected margin", "Margen esperado", "Marge attendue"],
  ["Break-even reward", "Recompensa de equilibrio", "Récompense au seuil de rentabilité"],
  ["FX provenance", "Procedencia del cambio", "Provenance du taux de change"],
  ["MARKET RATE", "TIPO DE MERCADO", "TAUX DE MARCHÉ"],
  ["USER ASSUMPTION", "SUPUESTO DEL OPERADOR", "HYPOTHÈSE DE L’OPÉRATEUR"],
  ["CONDITIONALLY PROFITABLE", "CONDICIONALMENTE RENTABLE", "CONDITIONNELLEMENT RENTABLE"],
  ["CONDITIONALLY MARGINAL", "CONDICIONALMENTE MARGINAL", "CONDITIONNELLEMENT MARGINAL"],
  ["CONDITIONALLY UNECONOMIC", "CONDICIONALMENTE NO RENTABLE", "CONDITIONNELLEMENT NON RENTABLE"],
  ["NOT ELIGIBLE", "NO ELEGIBLE", "NON ADMISSIBLE"],
  ["This result depends on observed inputs and your explicit assumptions. It is not realized profit.", "Este resultado depende de datos observados y de tus supuestos explícitos. No es un beneficio realizado.", "Ce résultat dépend de données observées et de vos hypothèses explicites. Il ne s’agit pas d’un profit réalisé."],
  ["USDC amounts remain exact source units. USD economics appear only with current FX, published pricing, and explicit operator assumptions.", "Los importes en USDC conservan las unidades exactas de la fuente. La economía en USD solo aparece con un tipo de cambio vigente, precios publicados y supuestos explícitos del operador.", "Les montants USDC conservent les unités exactes de la source. Les calculs en USD n’apparaissent qu’avec un taux de change actuel, des prix publiés et des hypothèses explicites de l’opérateur."],
  [
    "Eligibility has changed. Recheck the current source constraints.",
    "La elegibilidad ha cambiado. Vuelve a comprobar las restricciones actuales de la fuente.",
    "L’admissibilité a changé. Vérifiez à nouveau les contraintes actuelles de la source.",
  ],
  [
    "Market observations are temporarily unavailable.",
    "Las observaciones del mercado no están disponibles temporalmente.",
    "Les observations du marché sont temporairement indisponibles.",
  ],
  [
    "Inspect this opportunity",
    "Examinar esta oportunidad",
    "Examiner cette opportunité",
  ],
  ["Observed at", "Observado el", "Observé le"],
  ["Apply policy", "Aplicar política", "Appliquer la politique"],
  ["Retry underwriting", "Reintentar evaluación", "Réessayer l’analyse"],
  [
    "UNDERWRITING RESULT",
    "RESULTADO DE LA EVALUACIÓN",
    "RÉSULTAT DE L’ANALYSE",
  ],
  [
    "A source or policy constraint blocks this opportunity.",
    "Una restricción de la fuente o de la política bloquea esta oportunidad.",
    "Une contrainte de la source ou de la politique bloque cette opportunité.",
  ],
  [
    "The evidence is not complete enough to recommend this work.",
    "La evidencia es insuficiente para recomendar esta tarea.",
    "Les éléments disponibles ne suffisent pas à recommander cette mission.",
  ],
  [
    "What holds this decision back",
    "Qué impide una recomendación",
    "Ce qui empêche une recommandation",
  ],
  ["Exact reason codes", "Códigos de motivo exactos", "Codes de motif exacts"],
  [
    "The submission deadline has passed.",
    "El plazo de entrega ha vencido.",
    "La date limite de soumission est passée.",
  ],
  [
    "The source does not provide a usable submission deadline.",
    "La fuente no proporciona un plazo de entrega válido.",
    "La source ne fournit pas de date limite de soumission exploitable.",
  ],
  [
    "The scoring window has closed, even if the listing remains visible.",
    "El periodo de puntuación ha cerrado, aunque el anuncio siga visible.",
    "La période d’évaluation est terminée, même si l’annonce reste visible.",
  ],
  [
    "The source has not reported a committed payment.",
    "La fuente no ha declarado un pago comprometido.",
    "La source n’a pas signalé de paiement engagé.",
  ],
  [
    "The source has not confirmed that verification is ready.",
    "La fuente no ha confirmado que la verificación esté lista.",
    "La source n’a pas confirmé que la vérification est prête.",
  ],
  [
    "Participation requires funding activity outside SignalForge’s permitted scope.",
    "Participar requiere financiación fuera del alcance permitido de SignalForge.",
    "La participation exige un financement hors du périmètre autorisé de SignalForge.",
  ],
  [
    "The complete task cannot be mapped to a supported fulfillment route.",
    "La tarea completa no puede asignarse a una ruta de ejecución compatible.",
    "La mission complète ne peut pas être associée à un itinéraire de réalisation pris en charge.",
  ],
  [
    "A complete, bounded fulfillment specification is still needed.",
    "Falta una especificación completa y acotada de la tarea.",
    "Une spécification complète et bornée de la mission reste nécessaire.",
  ],
  [
    "Platform, verification and other fulfillment fees are not fully known.",
    "No se conocen todos los costes de plataforma, verificación y ejecución.",
    "Les frais de plateforme, de vérification et de réalisation ne sont pas tous connus.",
  ],
  [
    "The source’s funding and eligibility statements require independent confirmation.",
    "Las declaraciones de financiación y elegibilidad requieren confirmación independiente.",
    "Les déclarations de financement et d’admissibilité de la source nécessitent une confirmation indépendante.",
  ],
  [
    "The USDC reward and USD provider prices have not been converted or combined.",
    "La recompensa en USDC y los precios en USD no se han convertido ni combinado.",
    "La récompense en USDC et les prix des prestataires en USD n’ont été ni convertis ni combinés.",
  ],
  [
    "The reward is known in USDC; a USD exchange assumption has not been supplied.",
    "La recompensa se conoce en USDC; no se ha indicado un tipo de cambio a USD.",
    "La récompense est connue en USDC ; aucune hypothèse de change en USD n’a été fournie.",
  ],
  [
    "There is no measured probability of winning or completing this work.",
    "No existe una probabilidad medida de ganar o completar esta tarea.",
    "Il n’existe aucune probabilité mesurée de remporter ou de terminer cette mission.",
  ],
  [
    "A supported task and explicit token/call limits are needed for a provider cost ceiling.",
    "Para acotar el coste del proveedor se necesitan una tarea compatible y límites explícitos de tokens y llamadas.",
    "Un plafond de coût exige une mission prise en charge et des limites explicites de jetons et d’appels.",
  ],
  [
    "Underwrite agent work.",
    "Evalúa el trabajo de agentes.",
    "Évaluez les missions d’agents.",
  ],
  ["Keep the evidence.", "Conserva la evidencia.", "Conservez les preuves."],
  [
    "Observed opportunities, explicit economic gaps, and auditable receipts. No marketplace actions. Every contract states execution_not_enabled.",
    "Oportunidades observadas, carencias económicas explícitas y recibos auditables. Sin acciones en mercados. Cada contrato indica execution_not_enabled.",
    "Opportunités observées, lacunes économiques explicites et reçus auditables. Aucune action sur les marchés. Chaque contrat indique execution_not_enabled.",
  ],
  [
    "Returns a capability contract and observed supply. Optional Groq interprets objectives; deterministic code preserves unknown costs and incomplete coverage.",
    "Devuelve un contrato de capacidades y oferta observada. Groq puede interpretar objetivos; el código determinista conserva los costes desconocidos y la cobertura incompleta.",
    "Renvoie un contrat de capacités et l’offre observée. Groq peut interpréter les objectifs ; le code déterministe préserve les coûts inconnus et la couverture incomplète.",
  ],
  ["Search", "Buscar", "Rechercher"],
  ["Success probability", "Probabilidad de éxito", "Probabilité de réussite"],
  [
    "Capability coverage",
    "Cobertura de capacidades",
    "Couverture des capacités",
  ],
  [
    "AGENT WORK UNDERWRITER",
    "EVALUACIÓN ECONÓMICA DE TAREAS",
    "ANALYSE ÉCONOMIQUE DES MISSIONS",
  ],
  [
    "Find profitable AI-agent work.",
    "Encuentra trabajo rentable para agentes.",
    "Trouvez des missions rentables pour agents.",
  ],
  [
    "Discover paid work. Price a fulfillment route. See whether the economics hold.",
    "Descubre tareas remuneradas. Calcula el coste de una ruta. Evalúa su viabilidad económica.",
    "Découvrez des missions rémunérées. Chiffrez un itinéraire. Évaluez sa viabilité économique.",
  ],
  ["Inspect paid work", "Explorar tareas remuneradas", "Explorer les missions"],
  ["How it works", "Cómo funciona", "Fonctionnement"],
  [
    "CURRENT MARKET SNAPSHOT",
    "ESTADO ACTUAL DEL MERCADO",
    "APERÇU ACTUEL DU MARCHÉ",
  ],
  [
    "No qualifying work in this snapshot.",
    "No hay tareas aptas en esta instantánea.",
    "Aucune mission admissible dans cet aperçu.",
  ],
  [
    "Unknown inventory stays unknown. No demonstration tasks are substituted.",
    "La disponibilidad desconocida sigue siendo desconocida. No se sustituye por tareas ficticias.",
    "Une disponibilité inconnue reste inconnue. Aucune mission de démonstration ne la remplace.",
  ],
  [
    "observed opportunities",
    "oportunidades observadas",
    "opportunités observées",
  ],
  ["Observed reward", "Recompensa observada", "Récompense observée"],
  [
    "Funding is source-reported. Costs and eligibility still require review.",
    "La financiación la declara la fuente. Los costes y la elegibilidad requieren revisión.",
    "Le financement est déclaré par la source. Les coûts et l’admissibilité restent à examiner.",
  ],
  ["OBSERVED INPUTS", "DATOS OBSERVADOS", "DONNÉES OBSERVÉES"],
  ["DETERMINISTIC POLICY", "POLÍTICA DETERMINISTA", "POLITIQUE DÉTERMINISTE"],
  [
    "UNKNOWN STAYS UNKNOWN",
    "LO DESCONOCIDO NO SE INVENTA",
    "L’INCONNU RESTE INCONNU",
  ],
  ["EXECUTION DISABLED", "EJECUCIÓN DESHABILITADA", "EXÉCUTION DÉSACTIVÉE"],
  ["FIND", "ENCONTRAR", "TROUVER"],
  ["PRICE", "CALCULAR", "CHIFFRER"],
  ["DECIDE", "DECIDIR", "DÉCIDER"],
  [
    "Observe paid work and its funding constraints.",
    "Observa tareas remuneradas y sus condiciones de financiación.",
    "Observez les missions rémunérées et leurs contraintes de financement.",
  ],
  [
    "Separate published prices, bounded estimates and missing costs.",
    "Distingue precios publicados, estimaciones acotadas y costes desconocidos.",
    "Distinguez prix publiés, estimations bornées et coûts inconnus.",
  ],
  [
    "Inspect the policy decision and auditable contract.",
    "Revisa la decisión de la política y el contrato auditable.",
    "Examinez la décision de politique et le contrat auditable.",
  ],
  [
    "Built for operators. Readable by agents.",
    "Para operadores. Comprensible para agentes.",
    "Pour les opérateurs. Lisible par les agents.",
  ],
  ["Inspect the contract", "Inspeccionar el contrato", "Examiner le contrat"],
  ["OBSERVED MARKET", "MERCADO OBSERVADO", "MARCHÉ OBSERVÉ"],
  [
    "Price the work. Know the gaps.",
    "Calcula el coste. Identifica lo que falta.",
    "Chiffrez la mission. Identifiez les lacunes.",
  ],
  [
    "Source-reported funding. Independent underwriting. Execution disabled.",
    "Financiación declarada por la fuente. Evaluación independiente. Ejecución deshabilitada.",
    "Financement déclaré par la source. Analyse indépendante. Exécution désactivée.",
  ],
  [
    "Market snapshot unavailable. Last known data is retained.",
    "Instantánea no disponible. Se conservan los últimos datos conocidos.",
    "Aperçu indisponible. Les dernières données connues sont conservées.",
  ],
  [
    "Underwriting unavailable. Please try again shortly.",
    "Evaluación no disponible. Inténtalo de nuevo en unos momentos.",
    "Analyse indisponible. Réessayez dans quelques instants.",
  ],
  ["Minimum reward", "Recompensa mínima", "Récompense minimale"],
  ["Payment state", "Estado del pago", "État du paiement"],
  [
    "Source reports escrow",
    "Depósito declarado por la fuente",
    "Séquestre déclaré par la source",
  ],
  ["Current deadline", "Plazo vigente", "Échéance en cours"],
  [
    "Bounded source sample",
    "Muestra acotada de la fuente",
    "Échantillon borné de la source",
  ],
  [
    "More records exist; this view is limited to 20.",
    "Existen más registros; esta vista se limita a 20.",
    "D’autres enregistrements existent ; cette vue est limitée à 20.",
  ],
  ["Inspect the network", "Inspeccionar la red", "Examiner le réseau"],
  [
    "Observed opportunity radar",
    "Radar de oportunidades observadas",
    "Radar des opportunités observées",
  ],
  ["Reward", "Recompensa", "Récompense"],
  ["Known cost", "Coste conocido", "Coût connu"],
  ["Est. route cost", "Coste estimado de ruta", "Coût estimé de l’itinéraire"],
  ["Expected value", "Valor esperado", "Valeur attendue"],
  ["Source-reported", "Declarado por la fuente", "Déclaré par la source"],
  [
    "Opportunity inspector",
    "Inspector de oportunidades",
    "Inspection de l’opportunité",
  ],
  [
    "PRE-EXECUTION ESTIMATE",
    "ESTIMACIÓN PREVIA A LA EJECUCIÓN",
    "ESTIMATION AVANT EXÉCUTION",
  ],
  ["Original source", "Fuente original", "Source d’origine"],
  ["Economic envelope", "Resumen económico", "Enveloppe économique"],
  [
    "Known required spend",
    "Gasto requerido conocido",
    "Dépense requise connue",
  ],
  [
    "Refundable bond at risk",
    "Garantía reembolsable en riesgo",
    "Caution remboursable à risque",
  ],
  [
    "Actual outcome observations",
    "Resultados reales observados",
    "Résultats réels observés",
  ],
  [
    "USDC amounts are exact source units. No USD conversion, fee completeness or winning probability is assumed.",
    "Los importes USDC usan unidades exactas de la fuente. No se presupone conversión a USD, costes completos ni probabilidad de ganar.",
    "Les montants USDC sont les unités exactes de la source. Aucune conversion USD, exhaustivité des frais ou probabilité de gain n’est présumée.",
  ],
  [
    "Eligibility and constraints",
    "Elegibilidad y restricciones",
    "Admissibilité et contraintes",
  ],
  ["Verifier", "Verificador", "Vérificateur"],
  [
    "Policy is evaluated server-side against the full bounded snapshot.",
    "La política se evalúa en el servidor con la instantánea acotada completa.",
    "La politique est évaluée côté serveur sur l’intégralité de l’instantané borné.",
  ],
  [
    "Download underwriting JSON",
    "Descargar evaluación JSON",
    "Télécharger l’analyse JSON",
  ],
  ["Machine contract", "Contrato para agentes", "Contrat machine"],
  [
    "Checking economic inputs…",
    "Comprobando datos económicos…",
    "Vérification des données économiques…",
  ],
  [
    "Source evidence requirements",
    "Requisitos de evidencia de la fuente",
    "Exigences de preuve de la source",
  ],
  [
    "No claim, submission, payment or execution is authorized.",
    "No se autoriza reclamar, entregar, pagar ni ejecutar.",
    "Aucune prise en charge, soumission, dépense ou exécution n’est autorisée.",
  ],
  [
    "Find. Price. Decide.",
    "Encuentra. Calcula. Decide.",
    "Trouvez. Chiffrez. Décidez.",
  ],
  ["Opportunity ID", "ID de oportunidad", "Identifiant de l’opportunité"],
  [
    "Public snapshots and operational privacy",
    "Instantáneas públicas y privacidad operativa",
    "Instantanés publics et confidentialité opérationnelle",
  ],
  [
    "Upstash stores bounded public source snapshots, validator metadata, connector health, expiring refresh/model-admission leases and rate-limit counters. Caller keys are salted HMACs; SignalForge does not store raw IP addresses in these records.",
    "Upstash almacena instantáneas públicas acotadas, validadores, salud de conectores, bloqueos temporales de actualización y acceso al modelo, y contadores de solicitudes. Las claves usan HMAC con sal; SignalForge no almacena IP sin procesar en estos registros.",
    "Upstash conserve des instantanés publics bornés, des validateurs, l’état des connecteurs, des verrous temporaires de rafraîchissement et d’accès au modèle, et des compteurs de requêtes. Les identifiants sont des HMAC salés ; SignalForge n’y stocke pas d’adresses IP brutes.",
  ],
  [
    "Only optional objective decomposition sends your objective, context URL, budget, policy and language preference to Groq. The URL is text, not fetched. Marketplace descriptions never enter this model path. No tools, credentials or server configuration are included in the prompt.",
    "Solo la descomposición opcional envía a Groq tu objetivo, URL contextual, presupuesto, política e idioma. La URL es texto, no se consulta. Las descripciones del mercado nunca entran en este flujo. El mensaje no incluye herramientas, credenciales ni configuración del servidor.",
    "Seule la décomposition facultative envoie à Groq votre objectif, URL de contexte, budget, politique et langue. L’URL est du texte, elle n’est pas consultée. Les descriptions de missions ne suivent jamais ce chemin. Aucun outil, identifiant secret ou paramètre serveur n’est inclus.",
  ],
  [
    "SignalForge does not intentionally persist visitor objectives, scenarios or outcomes. Vercel may retain request metadata and operational logs; Groq may retain provider-side logs under its policies. Do not submit confidential text. No marketplace credentials, wallet credentials or private keys are requested; autonomous signing is not available.",
    "SignalForge no persiste intencionalmente objetivos, escenarios ni resultados de visitantes. Vercel puede conservar metadatos y registros operativos; Groq puede conservar registros según sus políticas. No envíes texto confidencial. No se solicitan credenciales de mercados, carteras ni claves privadas; no existe firma autónoma.",
    "SignalForge ne conserve volontairement ni objectifs, ni scénarios, ni résultats des visiteurs. Vercel peut conserver des métadonnées et journaux opérationnels ; Groq peut conserver des journaux selon ses politiques. N’envoyez pas de texte confidentiel. Aucun identifiant de place de marché, de portefeuille ou clé privée n’est demandé ; la signature autonome n’est pas disponible.",
  ],
];
